<template>
    <AppPage
        title="Laporan Pajak Fixed (DPKAD)"
        :breadcrumb="['Laporan', 'Laporan Pajak Fixed']"
    >
        <div class="space-y-1">
            <!-- Filters -->
            <div
                    ...
                    <div class="flex justify-between items-center px-2">
                        <div class="flex flex-col">
                            <span
                                ...
                                >Rekapitulasi Penjualan Terlapor Fixed (TAX)</span
                            >
                            <span
                                class="text-xs text-slate-400 font-medium uppercase tracking-wider"
                                >Membaca data langsung dari database DPKAD (resto_dpkad)</span
                            >
                        </div>
                    ...
                    >
                        <template #empty>
                            <div class="text-center py-12">
                                <i
                                    class="pi pi-info-circle text-4xl text-slate-200 mb-3 block"
                                ></i>
                                <div class="text-slate-400">
                                    Tidak ada data transaksi di database DPKAD
                                    untuk periode ini.
                                </div>
                            </div>
                        </template>
                        ...
                        >
                            <template #body="{ data }">
                                <span class="text-slate-600"
                                    >Rp
                                    {{ money(data.categories[cat] || 0) }}</span
                                >
                            </template>
                        </Column>
                        ...
                        <Column
                            field="subtotal"
                            header="Subtotal"
                            class="text-right font-bold bg-slate-50/50"
                        >
                            <template #body="{ data }">
                        ...
                        <Column
                            field="tax"
                            header="Pajak (TAX)"
                            class="text-right text-primary-600 font-bold"
                        >
                            <template #body="{ data }">
                                Rp {{ money(data.tax) }}
                            </template>
                        ...
                        <Column
                            field="grand_total"
                            header="Grand Total"
                            class="text-right font-black text-slate-900 bg-primary-50/30"
                        >
                            <template #body="{ data }">
                        ...
                        <Column
                            header="PB1 (Perkiraan)"
                            class="text-right font-bold text-emerald-600 bg-emerald-50/30"
                        >
                            <template #body="{ data }">
                                Rp {{ money((data.subtotal * 40) / 100) }}
                            </template>
                        </Column>

                        <template #footer>
                            <div
                                class="grid grid-cols-1 md:grid-cols-5 gap-6 p-4 bg-slate-900 rounded-xl text-white"
                            >
                                <div class="flex flex-col">
                                    <span
                                ...
                                >
                                    <span
                                        class="text-[10px] text-primary-400 uppercase font-bold tracking-widest"
                                        >Total Pajak (PB1)</span
                                    >
                                    <span
                                        class="text-xl font-black text-primary-400"
                                        >Rp {{ money(totals.tax) }}</span
                                    >
                                </div>
                                <div
                                    class="flex flex-col border-l border-slate-700 pl-6"
                                ...
                                >
                                    <span
                                        class="text-[10px] text-slate-400 uppercase font-bold tracking-widest"
                                        >Total Pajak PB1 (Perkiraan)</span
                                    >
                                    <span
                                        class="text-xl font-black text-emerald-400"
                                        >Rp
                                        {{
                                            money((totals.subtotal * 40) / 100)
                                        }}</span
                                    >
                                </div>
                            </div>
...
async function loadReport() {
    ...
    try {
        ...
        const response = await axios.get("/reports/tax-fixed", { params });
        reportData.value = response.data.data || [];
    } catch (error) {
        toast.add({
            severity: "error",
            summary: "Gagal",
            detail: "Gagal memuat laporan pajak.",
            life: 3000,
        });
    } finally {
        loading.value = false;
...
async function exportExcel() {
    await downloadFile("/reports/export/tax-fixed-excel", `Laporan_Pajak_Fixed`);
}

async function exportPdf() {
    await downloadFile("/reports/export/tax-fixed-pdf", `Laporan_Pajak_Fixed`, "pdf");
}
