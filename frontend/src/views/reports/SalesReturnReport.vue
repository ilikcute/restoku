<template>
  <AppPage :title="$t('sidebar.sales_return_report')" :breadcrumb="[$t('common.reports'), $t('sidebar.sales_return_report')]">
      ...
      <Card class="border-none shadow-sm">
        <template #title>
          <div class="flex justify-between items-center px-2">
            <span class="text-slate-800 font-black tracking-tight text-xl">Daftar Riwayat Retur Penjualan</span>
          </div>
        </template>
        <template #content>
          <DataTable :value="filteredReturns" paginator :rows="10" class="p-datatable-sm" stripedRows responsiveLayout="scroll">
            <template #empty>
              <div class="text-center py-8 text-slate-400">Tidak ada data retur penjualan untuk periode ini.</div>
            </template>
            <Column field="DT_RowIndex" header="No." class="text-center w-12">
              <template #body="{ index }">{{ index + 1 }}</template>
            </Column>
            <Column field="order_number" header="No. Pesanan" />
            <Column header="Tgl Retur">
              <template #body="{ data }">{{ formatDateTime(data.return_date) }}</template>
            </Column>
            <Column header="Pelanggan">
              <template #body="{ data }">{{ data.customer_name || 'Pelanggan Umum' }}</template>
            </Column>
            <Column header="Kasir">
...
async function loadData() {
  ...
  try {
    const response = await axios.get('/reports/sales-returns', { params });
    returns.value = response.data.data || [];
  } catch (error) {
    console.error('Failed to load return data', error);
    toast.add({ severity: 'error', summary: 'Gagal', detail: 'Gagal mengambil data retur penjualan.', life: 3000 });
  } finally {
    loading.value = false;
  }
}
...
function viewDetail(item) {
  // item adalah objek retur, kita butuh order_id-nya
