<template>
  <AppPage :title="$t('sidebar.purchase_report')" :breadcrumb="[$t('common.reports'), $t('sidebar.purchase_report')]">
        ...
        <template #content>
          <div class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex flex-col gap-2 flex-1 min-w-[200px]">
              <label class="text-xs font-bold text-slate-500 uppercase">Cari Pembelian</label>
      ...
      <Card class="border-none shadow-sm">
        <template #title>
          <div class="flex justify-between items-center px-2">
            <span class="text-slate-800 font-black tracking-tight text-xl">Data Riwayat Pembelian Stok</span>
          </div>
        </template>
        <template #content>
          <DataTable :value="filteredPurchases" paginator :rows="10" class="p-datatable-sm" stripedRows responsiveLayout="scroll">
            <template #empty>
              <div class="text-center py-8 text-slate-400">Tidak ada data pembelian untuk periode ini.</div>
            </template>
            ...
            <Column header="Tanggal">
              <template #body="{ data }">{{ formatDateTime(data.purchase_date) }}</template>
            </Column>
            ...
            <template #footer>
              <div class="flex justify-end pr-12 gap-4">
                <span class="font-bold text-slate-600 uppercase tracking-wider">Total Pembelian:</span>
                <span class="font-black text-xl text-primary-600">Rp {{ money(totalAmount) }}</span>
              </div>
            </template>
          </DataTable>
        </template>
...
const totalAmount = computed(() => {
  return filteredPurchases.value.reduce((sum, item) => sum + Number(item.total_amount), 0);
});
...
async function loadData() {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Gagal', detail: 'Gagal mengambil data pembelian.', life: 3000 });
  } finally {
    loading.value = false;
  }
}
