<template>
    <AppPage
        ...
        :breadcrumb="['Pengaturan', 'Promosi']"
        no-card
    >
        <template #actions>
            <Button
                :label="`${$t('common.add')} ${$t('promotions')}`"
                icon="pi pi-plus"
        ...
        <div class="space-y-1">
                        ...
                        >
                            <template #body="slotProps">
                                ...
                                >
                                    {{
                                        getApplicableLabel(
                                            slotProps.data.applicable_type,
                                        )
                                    }}
                                </span>
                            </template>
            ...
            <Dialog
                ...
                :style="{ width: '600px' }"
                :header="item.id ? 'Edit Promosi' : 'Promosi Baru'"
                :modal="true"
                class="p-fluid !rounded-3xl overflow-hidden shadow-2xl"
                ...
                <div class="space-y-6">
                        ...
                        <div class="space-y-4">
                            <div class="flex flex-col gap-2">
                                ...
                                <InputText
                                    ...
                                    autofocus
                                    :class="{
                                        'p-invalid': submitted && !item.title,
                                    }"
                                    placeholder="Contoh: Promo Gila Gajian!"
                                    class="!rounded-xl !bg-slate-50 !border-slate-100 focus:!ring-emerald-500 !p-3"
                                />
                                <small
                                    class="p-error v-if='submitted && !item.title'"
                                    >Judul wajib diisi.</small
                                >
                            </div>
                            ...
                            <div class="flex flex-col gap-2">
                                <label
                                    for="min_purchase"
                                    class="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1"
                                    >Minimal Belanja (Opsional)</label
                                >
                                <InputNumber
                                    id="min_purchase"
                                    v-model="item.min_purchase"
                                    :min="0"
                                    mode="currency"
                                    currency="IDR"
                                    locale="id-ID"
                        ...
                        <div class="space-y-4">
                            <div class="flex flex-col gap-2">
                                <label
                                    for="applicable_type"
                                    class="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1"
                                    >Target Promosi</label
                                >
                                <Select
                                    id="applicable_type"
                                    v-model="item.applicable_type"
                                    :options="applicableTypes"
                                    optionLabel="label"
                                    optionValue="value"
                            ...
                            <div
                                v-if="item.applicable_type === 'products'"
                                class="flex flex-col gap-2"
                            >
                                <label
                                    for="product_ids"
                            ...
                            <div
                                v-if="item.applicable_type === 'categories'"
                                class="flex flex-col gap-2"
                            >
                                <label
                                    for="category_ids"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div
                                    class="flex flex-col gap-2 justify-center pt-5"
                                >
                                    <div
                            ...
                            >
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <label
                                        ...
                                        >Berlaku Kelipatan (Qty)</label
                                    >
                                </div>
                                <div class="flex items-center gap-3">
                                    <Checkbox
                                        id="is_stackable"
                                        v-model="item.is_stackable"
                                        :binary="true"
                                    />
                                    <label
                                        for="is_stackable"
                                        class="text-xs font-bold text-slate-600 cursor-pointer"
                                        >Dapat Ditumpuk (Promo Lain)</label
                                    >
                                </div>
                            </div>

                            <div
                                class="p-3 bg-blue-50 rounded-2xl border border-blue-100 space-y-3"
                            >
                                <p
                                    class="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] flex items-center gap-2"
                                >
                    ...
                    <div class="flex flex-col gap-2">
                        <label
                            for="content"
                            class="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1"
                            >Keterangan / Konten Promosi</label
                        >
                        <Textarea
                            id="content"
...
function formatDiscount(data) {
    ...
    if (data.type === "discount_fixed")
        return `Rp ${data.discount_value.toLocaleString()} OFF`;
    return "";
}
...
function editItem(data) {
    ...
    if (item.value.start_date)
        item.value.start_date = new Date(item.value.start_date);
    if (item.value.end_date)
        item.value.end_date = new Date(item.value.end_date);
    itemDialog.value = true;
}
