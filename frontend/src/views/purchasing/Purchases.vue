  ...
  <AppPage title="Pembelian Stok" :breadcrumb="['Manajemen', 'Pembelian']" no-card>
    ...
    <AppDataTable framed :value="rows" :loading="loading" paginator :rows="20" class="app-table"
      ...
      <Column field="payment_status" :header="$t('common.payment_status')" style="width: 120px">
        <template #body="{ data }">
          <div :class="[
            ...
            <span
              :class="['w-1.5 h-1.5 rounded-full', data.payment_status === 'paid' ? 'bg-emerald-500' : 'bg-rose-500']"></span>
      ...
      <Column field="status" :header="$t('common.status')" style="width: 120px">
        <template #body="{ data }">
          <div
            class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest inline-flex items-center gap-1.5 bg-slate-50 text-slate-600 border border-slate-100">
      ...
      <Column header="" class="text-right w-32">
        <template #body="{ data }">
          <div class="flex justify-end gap-2">
            <Button icon="pi pi-eye" text rounded severity="success" class="hover:bg-emerald-50"
              @click="viewDetail(data)" v-tooltip.top="'Detail'" />
            <Button icon="pi pi-print" text rounded severity="secondary" class="hover:bg-slate-100"
              @click="downloadPdf(data)" v-tooltip.top="'Print'" />
          </div>
        </template>
      </Column>
    </AppDataTable>
      ...
      <div class="space-y-6">
        ...
        <div class="flex items-start gap-4 p-4 bg-orange-50 rounded-2xl border border-orange-100">
          ...
          <div>
            ...
            <p class="text-[11px] text-orange-700/70 leading-relaxed font-medium">Input data pembelian barang untuk
              memperbarui stok dan mencatat pengeluaran secara otomatis.</p>
          </div>
        </div>
        ...
        <div class="grid gap-6 md:grid-cols-4">
          <div class="flex flex-col gap-2">
            ...
            <Select v-model="form.supplier_id" :options="suppliers" optionLabel="name" optionValue="id"
              :placeholder="$t('common.supplier')" class="!rounded-xl !bg-slate-50 !border-slate-100"
              pt:input:class="!p-3 !text-sm" />
          </div>
          <div class="flex flex-col gap-2">
            ...
            ]" optionLabel="label" optionValue="value" class="!rounded-xl !bg-slate-50 !border-slate-100"
              pt:input:class="!p-3 !text-sm" />
          </div>
          <div v-if="form.payment_method === 'cash'" class="flex flex-col gap-2">
            ...
            <Select v-model="form.account_id" :options="accounts" optionLabel="name" optionValue="id"
              :placeholder="$t('purchasing.account')" class="!rounded-xl !bg-slate-50 !border-slate-100"
              pt:input:class="!p-3 !text-sm" />
          </div>
          <div v-else class="flex flex-col gap-2">
            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1 opacity-40">Status
              Pembayaran</label>
            <div
              class="p-3 bg-rose-50 border border-rose-100 rounded-xl text-rose-600 font-bold text-xs flex items-center gap-2">
          ...
          <div class="flex flex-col gap-2">
            ...
            <DatePicker v-model="purchaseDate" dateFormat="yy-mm-dd" showIcon iconDisplay="input"
        ...
        <div class="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
          ...
          <div class="grid gap-4 md:grid-cols-4">
            <div class="flex flex-col gap-2 md:col-span-2">
              ...
              <Select v-model="item.product_id" :options="products" optionLabel="name" optionValue="id" filter
                :placeholder="$t('sidebar.products')" class="!rounded-xl overflow-hidden"
                pt:input:class="!bg-white !border-slate-200 !p-3 !text-sm" />
            </div>
            <div class="flex flex-col gap-2">
              <label class="text-[10px] font-bold text-slate-400 uppercase ml-1">Jumlah (Qty)</label>
              <InputNumber v-model="item.quantity" mode="decimal" :min="1" placeholder="0"
                class="!rounded-xl overflow-hidden" pt:input:class="!bg-white !border-slate-200 !p-3 !text-sm" />
            </div>
            <div class="flex flex-col gap-2">
              <label class="text-[10px] font-bold text-slate-400 uppercase ml-1">Harga Beli / Unit</label>
              <InputNumber v-model="item.cost_price" mode="decimal" :min="0" placeholder="0"
                class="!rounded-xl overflow-hidden" pt:input:class="!bg-white !border-slate-200 !p-3 !text-sm" />
            </div>
          </div>
          <div class="flex justify-end pt-2">
            <Button label="Tambahkan ke Daftar" icon="pi pi-plus" size="small"
              class="!rounded-xl !px-8 !bg-orange-600 !border-none font-bold shadow-md shadow-orange-100"
              @click="addItem" />
          </div>
        </div>
        ...
        <div class="flex justify-between items-center w-full px-2">
          ...
          <div class="flex gap-3">
            ...
            <Button :label="`${$t('common.save')}`" icon="pi pi-check" :loading="saving"
              class="!rounded-xl !bg-orange-600 !border-none !px-8 h-12 font-bold shadow-lg shadow-orange-100"
              @click="save" />
          </div>
        </div>
      </template>
