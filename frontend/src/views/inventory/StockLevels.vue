<template>
  <AppPage :title="$t('inventory.stock_levels')" :breadcrumb="[$t('common.inventory'), $t('inventory.stock_levels')]">
    <div class="space-y-6">
      <!-- Summary Cards -->
      ...
      <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-wrap gap-4 items-center justify-between">
        <div class="flex flex-wrap gap-3 items-center">
          <div class="p-input-icon-left">
            ...
            <InputText v-model="query" :placeholder="$t('inventory.search_product')" class="w-full md:w-64" />
          </div>
          <Select 
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        >
          ...
          <Column :header="$t('common.name')" field="name" sortable class="font-semibold" />
...
const filteredRows = computed(() => {
  return rows.value.filter((row) => {
    const matchSearch = row.name?.toLowerCase().includes(query.value.toLowerCase());
    return matchSearch;
  });
});
